...
const labelsEn: Record<Quantity, string> = { power: 'Active Power P', voltage: 'Voltage U', current: 'Current I', resistance: 'Resistance R', outputPower: 'Output Power Pout', inputPower: 'Input Power Pin', efficiency: 'Efficiency η' }
const quantityLabel = (key: Quantity): string => tr(labels[key], labelsEn[key])
const modeTitle = computed(() => ({ dc: tr('直流功率', 'DC Power'), resistive: tr('纯阻性负载', 'Resistive Load'), single: tr('单相交流', 'Single-phase AC'), three: tr('三相交流', 'Three-phase AC'), efficiency: tr('效率换算', 'Efficiency') })[mode.value])
...
const inputs = computed<InputDefinition[]>(() => {
  ...
  return keys.map((key) => ({ key, label: quantityLabel(key), units: units[key] }))
})
...
function format(value: number): string { return Number(value.toPrecision(9)).toLocaleString(language.value, { maximumFractionDigits: 9 }) }
</script>

<template>
  <section class="power-layout">
    <div class="panel power-calculator-panel">
      <div class="panel-toolbar"><div><h2>{{ tr('功率计算器', 'Power Calculator') }}</h2><p>{{ tr('直流、阻性负载、单相/三相交流与效率换算', 'DC, resistive, single/three-phase AC and efficiency calculations') }}</p></div><span class="circuit-type">{{ modeInfo.formula }}</span></div>
      <div class="power-mode-tabs">
        <button v-for="item in ([['dc',tr('直流','DC')],['resistive',tr('纯阻性','Resistive')],['single',tr('单相交流','Single-phase')],['three',tr('三相交流','Three-phase')],['efficiency',tr('效率','Efficiency')]] as const)" :key="item[0]" :class="{ active: mode === item[0] }" @click="switchMode(item[0])">{{ item[1] }}</button>
      </div>
      <h3 class="power-section-title">{{ tr('选择要计算的量', 'Choose a Target') }}</h3>
      <div class="solve-targets power-targets"><label v-for="item in modeInfo.targets" :key="item" :class="{ active: target === item }"><input v-model="target" type="radio" :value="item" @change="clearResult" />{{ tr('求', 'Solve ') }}{{ quantityLabel(item) }}</label></div>
      <label v-if="mode === 'resistive' && (target === 'power' || target === 'resistance')" class="resistive-source">{{ tr('已知方式', 'Known Value') }}<select v-model="resistivePair" @change="clearResult"><option value="voltage">{{ tr('已知电压', 'Voltage') }}</option><option value="current">{{ tr('已知电流', 'Current') }}</option></select></label>
      <div class="power-input-grid">
        <label v-for="input in inputs" :key="input.key"><span>{{ input.label }}</span><div class="value-with-unit"><input v-model="values[input.key]" type="number" min="0" step="any" :placeholder="tr('输入数值', 'Enter value')" @input="clearResult" /><select v-model.number="factors[input.key]" @change="clearResult"><option v-for="unit in input.units" :key="unit.label" :value="unit.factor">{{ unit.label }}</option></select></div></label>
        <label v-if="mode === 'single' || mode === 'three'"><span>{{ tr('功率因数 cosφ（0～1）', 'Power Factor cosφ (0–1)') }}</span><input v-model="powerFactor" type="number" min="0" max="1" step="0.01" @input="clearResult" /></label>
      </div>
      <button class="wide-action" @click="calculate">{{ tr('计算结果', 'Calculate') }}</button>
      <p v-if="error" class="error-message">{{ error }}</p>
      <div v-if="result !== null" class="unified-result"><span>{{ quantityLabel(target) }}</span><strong>{{ format(result) }} {{ resultUnit }}</strong><code>{{ modeInfo.formula }}</code></div>
    </div>
    <aside class="panel power-guide-panel">
      ...
      <div class="power-help"><strong>{{ tr('使用说明', 'Instructions') }}</strong><p v-if="mode === 'dc'">{{ tr('适用于直流电源、开发板和普通直流负载，可反算功率、电压或电流。', 'For DC supplies and loads; solve power, voltage or current.') }}</p><p v-else-if="mode === 'resistive'">{{ tr('适用于电阻、加热丝等纯阻性负载，可用电压或电流方式计算功率与电阻。', 'For resistors and heaters; calculate power and resistance from voltage or current.') }}</p><p v-else-if="mode === 'single'">{{ tr('适用于单相交流负载；功率因数为有功功率与视在功率之比。', 'For single-phase AC loads; power factor is active power divided by apparent power.') }}</p><p v-else-if="mode === 'three'">{{ tr('使用线电压和线电流计算平衡三相负载的有功功率。', 'Uses line voltage and current for balanced three-phase active power.') }}</p><p v-else>{{ tr('用于电源转换器、稳压器和电机系统的输入功率、输出功率与效率换算。', 'Convert input power, output power and efficiency for converters and motor systems.') }}</p></div>
      <div class="power-note"><strong>{{ tr('提示', 'Tip') }}</strong><p>{{ tr('计算结果统一以基础单位显示；输入框可直接选择 mW、kW、mV、mA、kΩ 等工程单位。', 'Results use base units; inputs support engineering units such as mW, kW, mV, mA and kΩ.') }}</p></div>
    </aside>
  </section>
</template>
