...
<template>
  <section class="media-downloader-layout">
    <div class="panel media-main">
      <div class="panel-toolbar"><div><h2>{{ tr('网页媒体下载', 'Web Media Downloader') }}</h2><p>{{ tr('粘贴公开网页地址，解析后下载视频或原始音频', 'Paste a public webpage URL, analyze it, then download video or original audio') }}</p></div><div class="media-tool-actions"><span class="virtual-status" :class="{ ready: status?.installed && status.ffmpegAvailable }">{{ status?.installed ? `yt-dlp ${status.version}` : tr('组件未安装', 'Component not installed') }} · FFmpeg {{ status?.ffmpegAvailable ? tr('已内置', 'Bundled') : tr('缺失', 'Missing') }}</span><button v-if="status?.installed" class="soft-button" :disabled="installing || progress.state === 'running'" @click="install">{{ installing ? tr('更新并校验中…', 'Updating and verifying…') : tr('更新 / 修复 yt-dlp', 'Update / Repair yt-dlp') }}</button><button v-if="status?.ffmpegAvailable" class="soft-button" :disabled="installingFfmpeg || progress.state === 'running'" @click="installFfmpeg">{{ installingFfmpeg ? tr('更新并校验中…', 'Updating and verifying…') : tr('更新 / 修复 FFmpeg', 'Update / Repair FFmpeg') }}</button></div></div>
      ...
      <template v-else>
        ...
        <label class="field"><span>{{ tr('网页链接或完整分享文案', 'Web URL or full share text') }}</span><div class="media-url-row"><input v-model="url" type="text" :placeholder="tr('粘贴公开网页链接或包含链接的分享文案（暂不支持抖音）', 'Paste a public URL or share text containing one (Douyin is not supported)')" @keyup.enter="analyze"><button :disabled="busy || !url.trim()" @click="analyze">{{ busy ? tr('解析中…', 'Analyzing…') : tr('解析链接', 'Analyze') }}</button></div></label>
