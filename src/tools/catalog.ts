...
export type ToolPageId = 'terminal' | 'waveform' | 'recording' | 'virtualPort' | 'ble' | 'usb' | 'networkProtocols' | 'mediaDownloader' | 'audioExtractor' | 'idPhoto' | 'baseConverter' | 'checksum' | 'csv' | 'ohms' | 'power' | 'network' | 'capacitors' | 'capconvert' | 'led' | 'opamp' | 'divider'
...
export const toolGroups: Array<{ title: string; titleEn: string; tools: ToolDefinition[] }> = [
  ...
  { title: '数据工具', titleEn: 'Data Tools', tools: [
    ...
    { id: 'idPhoto', label: '一寸照制作', labelEn: 'ID Photo Maker', icon: '照' },
