...
- `artifacts/DC Toolbox-1.1.2-x64-Setup.exe`：Windows 安装程序
- `artifacts/DC Toolbox-1.1.2-x64-Portable.exe`：免安装便携版
...
哔哩哔哩、YouTube 等网站通常分别提供视频轨和音频轨。用户可主动安装 FFmpeg-Builds 的 Windows x64 LGPL 组件，软件会依据 GitHub Release 提供的 SHA-256 摘要校验压缩包，然后自动下载并合并为 MP4。FFmpeg 不会被静默或强制安装。
